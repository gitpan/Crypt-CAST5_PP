...
# Delay loading s-boxes until first time init() is called.
# Is this a cheesy hack? AutoSplit doesn't define what will happen
# to things that aren't subs if they're after __END__.
use integer;
@s1 = (
...
sub init {
  ...
  # null-pad the key to 16 bytes, and split it into 32-bit chunks
...
sub encrypt {
  ...
  my ($l, $r) = unpack "N2", $block;
  my ($i, $j);
  my $k = $cast5->{key} or croak "Call init() first";

  # encrypt the data
  # don't try to read this -- it's generated by mkencrypt
  $i=$k->[0]+$r;$j=$k->[16];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$k->[1]^$l;$j=$k->[17];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$k->[2]-$r;$j=$k->[18];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$k->[3]+$l;$j=$k->[19];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$k->[4]^$r;$j=$k->[20];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$k->[5]-$l;$j=$k->[21];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$k->[6]+$r;$j=$k->[22];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$k->[7]^$l;$j=$k->[23];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$k->[8]-$r;$j=$k->[24];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$k->[9]+$l;$j=$k->[25];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$k->[10]^$r;$j=$k->[26];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$k->[11]-$l;$j=$k->[27];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  if ($cast5->{rounds} == 16) {
    $i=$k->[12]+$r;$j=$k->[28];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
    $i=$k->[13]^$l;$j=$k->[29];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
    $i=$k->[14]-$r;$j=$k->[30];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
    $i=$k->[15]+$l;$j=$k->[31];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  }
...
sub decrypt {
  ...
  my ($cast5, $block) = @_;
  croak "Block size must be 8" if length($block) != 8;
  my ($r, $l) = unpack "N2", $block;
  my ($i, $j);
  my $k = $cast5->{key} or croak "Call init() first";

  # decrypt the data
  # don't try to read this -- it's generated by mkdecrypt
  if ($cast5->{rounds} == 16) {
    $i=$k->[15]+$l;$j=$k->[31];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
    $i=$k->[14]-$r;$j=$k->[30];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
    $i=$k->[13]^$l;$j=$k->[29];{no integer;$i=($i<<$j)|(($i&0xffffffff)>>(32-$j))}$r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
